...
export default function Home() {
  ...
  const [newUser, setNewUser] = useState<newUsers>({ name: '', email: '' });
  ...
  useEffect(() => {
    fetch('http://localhost:4545/api/users')
      ...
      .then(data => {
        ...
        // Converti gli ID in numeri interi prima di ordinare
        const sortedData = data.sort((a: any, b: any) => parseInt(a.id) - parseInt(b.id));
        setUsers(sortedData);
      })
  ...
  const handleDeleteItem = () => {
    fetch(`http://localhost:4545/api/users/${userToDelete}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
              ...
              {users.map(user => (
                <tr key={user.id}>
                  ...
                  <td className='flex gap-3 p-2 justify-center'>
                    <Link href={`/user-details?id=${user.id}&name=${encodeURIComponent(user.name)}&email=${encodeURIComponent(user.email)}`}>
                    ...
                    <Link href={`/update?id=${user.id}&name=${encodeURIComponent(user.name)}&email=${encodeURIComponent(user.email)}`}>
                      <button className='font-bold bg-green-400 hover:bg-green-600 hover:cursor-pointer border-0 p-2 rounded-md text-white'>
                        Edit
                      </button>
                    </Link>
                    <button
            ...
            <div className="p-6 rounded shadow-lg modal text-white">
              ...
              <div className='flex flex-col gap-2'>
                ...
                <div className="flex justify-end">
                  <button
                    className=" font-bold bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded mr-2 hover:cursor-pointer"
                    onClick={() => setNewUserModal(false)}>Cancel</button>
                  <button
                    className=" font-bold bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded hover:cursor-pointer"
